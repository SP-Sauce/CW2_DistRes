...
All three nodes use `data\shared\users.db` and `data\shared\ProductSpecification.txt` in the local demo. This is important because the SQLite `resource_readers` and `resource_locks` tables must be shared for active-reader display and the database-backed distributed write lock to coordinate all nodes.
...
| `app/database.py` | Creates SQLite tables, seeded users, sessions, `resource_readers`, and `resource_locks`. |
